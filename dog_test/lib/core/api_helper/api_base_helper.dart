...
class ApiBaseHelper {
  ...
  Future<void> downloadImage(String imageUrl, List<String> imagePaths, String defaultImageUrl) async {
    try {
      final response = await http.get(Uri.parse(imageUrl));
      if (response.statusCode == 200) {
        final file = File(await _getImageFilePath(imageUrl));
      ...
      if (response.statusCode == 200) {
        final file = File(await _getImageFilePath(defaultImageUrl));
        await file.writeAsBytes(response.bodyBytes);
        imagePaths.add(file.path);
      }
    }
  ...
  Future<String> _getImageFilePath(String imageUrl) async {
    final dir = await Directory.systemTemp.create();
    return '${dir.path}/${imageUrl.hashCode}.jpg';
  }
